...
from crispr_screen_expert.exceptions import DataContractError, QualityControlError
...
def test_run_analysis_aborts_on_critical_qc(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualityControlError):
        run_analysis(
            config=config,
            paths=DataPaths(counts=counts_path, library=library_path),
...
def test_run_analysis_requires_metadata_when_config_missing(tmp_path: Path) -> None:
    counts_path = Path("sample_data/demo_counts.csv")
    library_path = Path("sample_data/demo_library.csv")

    with pytest.raises(DataContractError, match="Metadata path must be provided"):
        run_analysis(
            config=None,
            paths=DataPaths(counts=counts_path, library=library_path),
            settings=PipelineSettings(use_mageck=False, output_root=tmp_path, cache_annotations=False),
...
def test_run_analysis_uses_provided_config_without_metadata(
    tmp_path: Path, experiment_config, monkeypatch: pytest.MonkeyPatch
) -> None:
    def _raise_if_called(_path):
        raise AssertionError("load_metadata should not be invoked when config is supplied")

    monkeypatch.setattr("crispr_screen_expert.pipeline.load_metadata", _raise_if_called)
