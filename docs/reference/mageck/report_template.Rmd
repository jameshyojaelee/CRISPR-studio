...
<!--
  Adapted from the upstream MAGeCK manual report template. Stored under docs/reference
  so contributors can reproduce the default HTML report locally when needed.
-->
...
```{r}
fdrcutoff <- params$fdrcutoff
# Set fdrcutoff to -1 to disable the boundary line.
```
...
plotindvidualsgrnas_lfc<-function(sg_table,targetgene,collabel){
  ...
  if(ncol(targetmatvec_log)==length(collabel)){
    colnames(targetmatvec_log)=collabel
  }else{
    if(ncol(targetmatvec_log)==2){
      colnames(targetmatvec_log)=c('control','treatment')
    }
  }
